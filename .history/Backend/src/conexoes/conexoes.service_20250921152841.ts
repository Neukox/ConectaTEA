...
export class ConexoesService {
  ...
  async enviarSolicitacao(
    usuarioId: number,
    createConexaoDto: CreateConexaoDto
  ) {
    ...
    if (conexaoExistente) {
      throw new ConflictException(
        "Já existe uma conexão entre estes profissionais."
      );
    }
  ...
  async responderSolicitacao(
    usuarioId: number,
    conexaoId: number,
    updateConexaoDto: UpdateConexaoDto
  ) {
    const profissional = await this.prisma.profissional.findUnique({
      where: { usuario_id: usuarioId },
    });
    ...
    const novoStatus =
      updateConexaoDto.acao === AcaoConexao.ACEITAR
  ...
  async removerConexao(usuarioId: number, conexaoId: number) {
    ...
    if (
      conexao.solicitante_id !== profissional.id &&
      conexao.solicitado_id !== profissional.id
    ) {
      throw new ForbiddenException(
        "Você não tem permissão para remover esta conexão."
      );
    }
