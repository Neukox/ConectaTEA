...
export async function enviarSolicitacao(
  _solicitanteId: number,
  solicitadoId: number,
  options?: { tipo?: "user" | "prof" }
  ...
  try {
    ...
    const { data } = await api.post<{
      message: string;
      data: ConexaoProfissional;
    }>("/conexoes/enviar", body);
    return data.data;
  } catch (e) {
...
export async function aceitarSolicitacao(
  ...
  try {
    ...
    const { data } = await api.put<{
      message: string;
      data: ConexaoProfissional;
    }>(`/conexoes/${conexaoId}/responder`, body);
    return data.data;
  } catch (e) {
    throw new Error(extractErrorMessage(e));
  }
...
export async function listarSolicitacoesEnviadas(): Promise<
  ConexaoProfissional[]
> {
  try {
...
export async function listarSolicitacoesRecebidas(): Promise<
  ConexaoProfissional[]
> {
  try {
    const { data } = await api.get<{
...
// Listar conexões (aceitas) de um profissional (backend obtém do JWT)
export async function listarConexoesPorProfissional(): Promise<
  ConexaoProfissional[]
> {
  try {
    const { data } = await api.get<{
...
export async function obterConexaoPorProfissionais(
  ...
  try {
    const { data } = await api.get<{
      ...
      `/conexoes/filtrar?solicitanteProfId=${encodeURIComponent(
        solicitanteProfId
      )}&solicitadoProfId=${encodeURIComponent(solicitadoProfId)}`
    );
