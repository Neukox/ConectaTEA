...
export class CriancasController {
  ...
  @ApiOperation({ summary: "Obter código de vínculo de uma criança" })
  @ApiResponse({
    status: 200,
    description: "Código de vínculo obtido com sucesso.",
  })
  @ApiResponse({
    status: 404,
    description: "Criança ou código de vínculo não encontrado.",
  })
  @ApiBearerAuth()
  @UseGuards(JwtAuthGuard)
  @Get(":id/codigo-vinculo")
  async obterCodigoVinculo(@Param("id") id: string, @Req() req: any) {
    const criancaId = parseInt(id);
    if (isNaN(criancaId)) {
      throw new BadRequestException("ID deve ser um número válido.");
    }
    return await this.criancasService.obterCodigoVinculo(criancaId);
  }
