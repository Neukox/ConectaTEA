...
export async function listarConexoesPorProfissional(): Promise<ConexaoProfissional[]> {
  try {
    const { data } = await api.get<{ message: string; data: ConexaoProfissional[] }>(`/conexoes`);
    return data.data;
  } catch (e) {
    throw new Error(extractErrorMessage(e));
  }
